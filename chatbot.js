...
function addBot(html){
  ...
  row.innerHTML='<div class="xbav"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 18v-6a9 9 0 0 1 18 0v6"></path><path d="M21 19a2 2 0 0 1-2 2h-1a2 2 0 0 1-2-2v-3a2 2 0 0 1 2-2h3zM3 19a2 2 0 0 0 2 2h1a2 2 0 0 0 2-2v-3a2 2 0 0 0-2-2H3z"></path></svg></div><div class="xbub xbot-b">'+html+'</div>';
...
window.xbotProcess=function(text){
  ...
  setTimeout(function(){
    if(matched.length===0){ xbotAskAI(t); return; }
...
window.xbotAskAI=function(text){
  ...
  loadRow.innerHTML='<div class="xbav"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 18v-6a9 9 0 0 1 18 0v6"></path><path d="M21 19a2 2 0 0 1-2 2h-1a2 2 0 0 1-2-2v-3a2 2 0 0 1 2-2h3zM3 19a2 2 0 0 0 2 2h1a2 2 0 0 0 2-2v-3a2 2 0 0 0-2-2H3z"></path></svg></div><div class="xbub xbot-b" id="xbot-loading"><span style="color:#5A6A80;font-size:12px;">思考中...</span></div>';
  msgs.appendChild(loadRow); msgs.scrollTop=msgs.scrollHeight;

  var systemPrompt='你是「高雄城市學習網絡」的 AI 客服。你的任務是協助市民尋找終身學習資源。\n'+
    '本網站整合了高雄市9大局處、涵蓋38個行政區的學習管道。\n'+
    '主要分類包含：樂齡族群、職涯發展、永續環境、文化藝術。\n'+
    '回答規則：\n'+
    '1. 語氣專業、清晰，使用繁體中文。\n'+
    '2. 內容精煉，不要超過150字。\n'+
    '3. 當詢問學習需求時，引導他們使用「探索學習資源」功能。\n'+
    '4. 如果問題與學習無關，請禮貌拉回主題，告訴他們你專注於提供高雄市的終身學習資訊。';
  ...
  if(history.length>15) history=history.slice(0,1).concat(history.slice(history.length-14));
...
window.xbotToggle=function(){
  ...
  document.getElementById('xico-chat').style.display=opened?'none':'flex';
  document.getElementById('xico-close').style.display=opened?'inline':'none';
  document.getElementById('xbot-label').textContent=opened?'':'AI客服';
...
function init(){
  ...
  setTimeout(function(){
    addBot('您好！我是高雄城市學習網絡的 AI 客服 👋<br>本平台匯集了全市 9 大局處的終身學習資源。<br><br>請輸入您的需求（如：想學電腦、高齡課程），或直接點擊下方分類探索：<div class="xchips">'+
